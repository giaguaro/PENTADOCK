...
from mpnn.args import InterpretArgs
from mpnn.data import get_data_from_smiles, get_header, get_smiles, MoleculeDataLoader, MoleculeDataset
from mpnn.train import predict
from mpnn.utils import load_args, load_checkpoint, load_scalers, timeit
...
class mpnnModel:
    """A :class:`mpnnModel` is a wrapper around a :class:`~mpnn.models.model.MoleculeModel` for interpretation."""

    def __init__(self, args: InterpretArgs) -> None:
        """
        :param args: A :class:`~mpnn.args.InterpretArgs` object containing arguments for interpretation.
        """
        self.args = args
...
def mcts_rollout(node: MCTSNode,
    ...
    :param scoring_function: A function for scoring subgraph SMILES using a mpnn model.
...
def interpret(args: InterpretArgs) -> None:
    """
    Runs interpretation of a mpnn model using the Monte Carlo Tree Search algorithm.
    ...
    mpnn_model = mpnnModel(args)

    def scoring_function(smiles: List[str]) -> List[float]:
        return mpnn_model(smiles)[:, args.property_id - 1]
...
def mpnn_interpret() -> None:
    """Runs interpretation of a mpnn model.

    This is the entry point for the command line command :code:`mpnn_interpret`.
    """
