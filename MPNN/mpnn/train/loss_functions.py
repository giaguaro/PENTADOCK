...
from mpnn.args import TrainArgs
...
def get_loss_func(args: TrainArgs) -> Callable:
    ...
    supported_loss_functions = {
        "regression": {
            "mse": mse_loss_top, #nn.MSELoss(reduction="none"),
...
def rmse_loss_top(
    ...
    # Take the absolute values of predictions and targets
    predictions_abs = torch.abs(predictions)
    targets_abs = torch.abs(targets)

    # Calculate the RMSE loss on all predicted scores
    mse_loss = nn.functional.mse_loss(predictions_abs, targets_abs, reduction="none").sum(dim=0)
    rmse_loss = torch.sqrt(mse_loss / predictions.shape[0])

    # Calculate the RMSE loss on the top predicted scores only
    num_top = max(1, int(top_fraction * predictions.shape[0]))
    top_scores = torch.topk(predictions_abs, num_top, dim=0, largest=True).values
    top_targets = torch.topk(targets_abs, num_top, dim=0, largest=True).values
    top_mse_loss = nn.functional.mse_loss(top_scores, top_targets, reduction="none").sum(dim=0)
    top_rmse_loss = torch.sqrt(top_mse_loss / num_top)

    # Combine the losses
    alpha = 0.0  # Weight for the top loss
    biased_rmse_loss = alpha * top_rmse_loss + (1 - alpha) * rmse_loss

    return biased_rmse_loss
...
def r2_loss_top(
    ...
    residuals = predictions_abs - targets_abs
    SS_residual = (residuals ** 2).sum(dim=0)

    # Calculate the total sum of squares (SS_total)
    targets_mean = targets_abs.mean(dim=0)
    SS_total = ((targets_abs - targets_mean) ** 2).sum(dim=0)

    epsilon = 1e-8
    r2_loss = 1 - (SS_residual / (SS_total + epsilon))

    # Calculate the R2 loss on all predicted scores
    r2_loss = 1 - (SS_residual / (SS_total + epsilon))

    # Calculate the R2 loss on the top predicted scores only
    num_top = max(1, int(top_fraction * predictions.shape[0]))
    top_scores = torch.topk(predictions_abs, num_top, dim=0, largest=True).values
    top_targets = torch.topk(targets_abs, num_top, dim=0, largest=True).values
    top_residuals = top_scores - top_targets
    top_SS_residual = (top_residuals ** 2).sum(dim=0)
    top_SS_total = ((top_targets - top_targets.mean(dim=0)) ** 2).sum(dim=0)
    top_r2_loss = 1 - (top_SS_residual / (top_SS_total + epsilon))

    # Combine the losses
    alpha = 0.5  # Weight for the top loss
    biased_r2_loss = alpha * top_r2_loss + (1 - alpha) * r2_loss

    return biased_r2_loss
...
def mse_loss_top(
   ...
   # upper_bound: torch.tensor,
    ...
    # Take the absolute values of predictions and targets
    #predictions_clipped = torch.abs(predictions)
    #targets_abs = torch.abs(targets)

    # Clip the predictions using the lower and upper bounds
#    predictions_clipped = torch.where(predictions_abs < lower_bound, lower_bound, predictions_abs)
 #   predictions_clipped = torch.where(predictions_clipped > upper_bound, upper_bound, predictions_clipped)

    # Calculate the MSE loss on the top predicted scores only
    num_top = max(1, int(top_fraction * predictions.shape[0]))
    top_scores = torch.topk(predictions, num_top, dim=0, largest=False).values
    top_targets = torch.topk(targets, num_top, dim=0, largest=False).values
    top_loss = nn.functional.mse_loss(top_scores, top_targets, reduction="none").sum(dim=0)

    # Calculate the MSE loss on all predicted scores
    mse_loss = nn.functional.mse_loss(predictions, targets, reduction="none").sum(dim=0)

    # Combine the losses
    alpha = 0.1 # Weight for the top loss
    bounded_mse_loss = alpha * top_loss + (1 - alpha) * mse_loss

    return bounded_mse_loss
