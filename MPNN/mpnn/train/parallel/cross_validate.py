...
from mpnn.nn_utils import param_count, param_count_all
from mpnn.args import TrainArgs
from mpnn.constants import TEST_SCORES_FILE_NAME, TRAIN_LOGGER_NAME
from mpnn.data import get_data, get_task_names, MoleculeDataset, validate_dataset_type
from mpnn.utils import create_logger, makedirs, timeit, multitask_mean
from mpnn.features import set_extra_atom_fdim, set_extra_bond_fdim, set_explicit_h, set_adding_hs, set_keeping_atom_map, set_reaction, reset_featurization_parameters


@timeit(logger_name=TRAIN_LOGGER_NAME)
def cross_validate(args: TrainArgs, model,
    ...
    :param args: A :class:`~mpnn.args.TrainArgs` object containing arguments for
                 loading data and training the mpnn model.
    ...
    for fold_num in range(args.num_folds):
        ...
        # Otherwise, train the models
            ...
            model_scores = train_func(args, model, data, logger)
...
def mpnn_train() -> None:
    """Parses mpnn training arguments and trains (cross-validates) a mpnn model.

    This is the entry point for the command line command :code:`mpnn_train`.
    """

    debug = info = print

    os.environ['MASTER_ADDR'] = 'schrogpu1'
    os.environ['MASTER_PORT'] = '12355'
    os.environ['CUDA_LAUNCH_BLOCKING'] = '1'  # Added for debugging purposes
    args = TrainArgs().parse_args()

    # Set the local_rank to the appropriate GPU ID
    ...
    cross_validate(args=args, model=model, train_func=run_training)
