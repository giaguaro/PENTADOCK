...
import mpnn.data.utils
from mpnn.data import set_cache_mol, empty_cache
from mpnn.features import get_available_features_generators
...
class CommonArgs(Tap):
    ...
    local_rank: int = -1
    """
    Local rank of the process on the node (for DDP).
    """
    rank: int = -1
    """
    Local rank of the process on the node (for DDP).
    ...
    def device(self) -> torch.device:
        ...
        if self.local_rank in [0, 1]:
            ...
            raise ValueError("Invalid local_rank. It should be either 0 or 1.")
    ...
    def features_scaling(self) -> bool:
        """
        Whether to apply normalization with a :class:`~mpnn.data.scaler.StandardScaler`
    ...
    def configure(self) -> None:
        ...
        self.add_argument('--local-rank', type=int)
        self.add_argument('--rank', type=int)
...
class TrainArgs(CommonArgs):
    """:class:`TrainArgs` includes :class:`CommonArgs` along with additional arguments used for training a mpnn model."""
    ...
    with mpnn's default molecule or multi-molecule encoders, or in :code:`reaction_solvent` mode where it applies to the solvent only.
    """
    ...
    def atom_constraints(self) -> List[bool]:
        ...
        if self.is_atom_bond_targets and self.constraints_path:
            header = mpnn.data.utils.get_header(self.constraints_path)
    ...
    def process_args(self) -> None:
        ...
        self.smiles_columns = mpnn.data.utils.preprocess_smiles_columns(
            path=self.data_path,
        ...
        if self.is_atom_bond_targets:
            self.atom_targets, self.bond_targets, self.molecule_targets = mpnn.data.utils.get_mixed_task_names(
                path=self.data_path,
                smiles_columns=self.smiles_columns,
...
class PredictArgs(CommonArgs):
    """:class:`PredictArgs` includes :class:`CommonArgs` along with additional arguments used for predicting with a mpnn model."""
...
class InterpretArgs(CommonArgs):
    """:class:`InterpretArgs` includes :class:`CommonArgs` along with additional arguments used for interpreting a trained mpnn model."""
...
class HyperoptArgs(TrainArgs):
    """:class:`HyperoptArgs` includes :class:`TrainArgs` along with additional arguments used for optimizing mpnn hyperparameters."""
