...
from mpnn.args import HyperoptArgs
from mpnn.constants import HYPEROPT_LOGGER_NAME
from mpnn.models import MoleculeModel
from mpnn.nn_utils import param_count
from mpnn.train import cross_validate, run_training
from mpnn.utils import create_logger, makedirs, timeit
from mpnn.hyperopt_utils import merge_trials, load_trials, save_trials, \
...
def hyperopt(args: HyperoptArgs) -> None:
    """
    Runs hyperparameter optimization on a mpnn model.
    ...
    * :code:`ffn_num_layers`: The number of feed-forward layers after message passing is selected from {1, 2, 3}
    ...
    :param args: A :class:`~mpnn.args.HyperoptArgs` object containing arguments for hyperparameter
...
def mpnn_hyperopt() -> None:
    """Runs hyperparameter optimization for a mpnn model.

    This is the entry point for the command line command :code:`mpnn_hyperopt`.
    """
    world_size = torch.cuda.device_count()
    mp.spawn(hyperopt(args=HyperoptArgs().parse_args()), nprocs=world_size)
