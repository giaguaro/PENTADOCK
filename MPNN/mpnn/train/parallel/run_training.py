...
from mpnn.spectra_utils import normalize_spectra, load_phase_mask
from mpnn.args import TrainArgs
from mpnn.constants import MODEL_FILE_NAME
from mpnn.data import get_class_sizes, get_data, MoleculeDataLoader, MoleculeDataset, set_cache_graph, split_data
from mpnn.models import MoleculeModel
from mpnn.nn_utils import param_count, param_count_all
from mpnn.utils import build_optimizer, build_lr_scheduler, load_checkpoint, makedirs, \
    save_checkpoint, save_smiles_splits, load_frzn_model, multitask_mean


def run_training(args: TrainArgs, model,
                 data: MoleculeDataset,
                 logger: Logger = None) -> Dict[str, List[float]]:
    """
    Loads data, trains a mpnn model, and returns test scores for the model checkpoint with the highest validation score.

    :param args: A :class:`~mpnn.args.TrainArgs` object containing arguments for
                 loading data and training the mpnn model.
    :param data: A :class:`~mpnn.data.MoleculeDataset` containing the data.
 ...
 #   torch.cuda.set_device(args.local_rank)
  ...
  #  args.device = torch.device("cuda:{}".format(local_rank))
    ...
    if len(val_data) == 0:
        raise ValueError('The validation data split is empty. During normal mpnn training (non-sklearn functions), \
