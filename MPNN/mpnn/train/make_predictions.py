...
from mpnn.args import PredictArgs, TrainArgs
from mpnn.data import get_data, get_data_from_smiles, MoleculeDataLoader, MoleculeDataset, StandardScaler, AtomBondScaler
from mpnn.utils import load_args, load_checkpoint, load_scalers, makedirs, timeit, update_prediction_args
from mpnn.features import set_extra_atom_fdim, set_extra_bond_fdim, set_reaction, set_explicit_h, set_adding_hs, set_keeping_atom_map, reset_featurization_parameters
from mpnn.models import MoleculeModel
from mpnn.uncertainty import UncertaintyCalibrator, build_uncertainty_calibrator, UncertaintyEstimator, build_uncertainty_evaluator
from mpnn.multitask_utils import reshape_values
...
def load_model(args: PredictArgs, generator: bool = False):
    ...
    :param args: A :class:`~mpnn.args.PredictArgs` object containing arguments for
...
def load_data(args: PredictArgs, smiles: List[List[str]]):
    ...
    :return: A tuple of a :class:`~mpnn.data.MoleculeDataset` containing all datapoints, a :class:`~mpnn.data.MoleculeDataset` containing only valid datapoints,
                 a :class:`~mpnn.data.MoleculeDataLoader` and a dictionary mapping full to valid indices.
    """
    print("Loading data")
    ...
    pickle.dump(open("chunk_pred.pkl", "wb"), test_data)
...
def set_features(args: PredictArgs, train_args: TrainArgs):
    ...
    :param args: A :class:`~mpnn.args.PredictArgs` object containing arguments for
                 loading data and a model and making predictions.
    :param train_args: A :class:`~mpnn.args.TrainArgs` object containing arguments for training the model.
    """
...
def predict_and_save(
    ...
    :param args: A :class:`~mpnn.args.PredictArgs` object containing arguments for
                 loading data and a model and making predictions.
    :param train_args: A :class:`~mpnn.args.TrainArgs` object containing arguments for training the model.
    :param test_data: A :class:`~mpnn.data.MoleculeDataset` containing valid datapoints.
    :param task_names: A list of task names.
    :param num_tasks: Number of tasks.
    :param test_data_loader: A :class:`~mpnn.data.MoleculeDataLoader` to load the test data.
    :param full_data:  A :class:`~mpnn.data.MoleculeDataset` containing all (valid and invalid) datapoints.
    :param full_to_valid_indices: A dictionary dictionary mapping full to valid indices.
    :param models: A list or generator object of :class:`~mpnn.models.MoleculeModel`\ s.
    :param scalers: A list or generator object of :class:`~mpnn.features.scaler.StandardScaler` objects.
    :param num_models: The number of models included in the models and scalers input.
    :param calibrator: A :class: `~mpnn.uncertainty.UncertaintyCalibrator` object, for use in calibrating uncertainty predictions.
...
def mpnn_predict() -> None:
    """Parses mpnn predicting arguments and runs prediction using a trained mpnn model.

    This is the entry point for the command line command :code:`mpnn_predict`.
    """
